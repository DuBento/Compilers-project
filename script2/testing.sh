...
if [ $# -ge 1 ]; then
        ...
        TESTS=`${LS} ${TEST_DIR}*${COMPILER_EXT} | sort -V`
fi

${DIFF} <( printf '%s' "$2") <( echo 'valgrind' )
if [ $# = 2 -a $? -eq 0 ]; then
...
do
        ...
        do
                ...
                ${useValgrind} ./fir -g ${inputTest} -o ${targetOut} --target ${target} 2>> ${testErr}
                rvBuild=$?
                if [ ${rvBuild} = 139 ]; then
        ...
        targetIn=${INPUT_DIR}${testName}${INPUT_EXT}
        ...
        echo ----exec >> ${testErr}
        if [ -f ${targetIn} ]; then
                ${useValgrind} ./${targetExe} < ${targetIn} > ${targetOut} 2>> ${testErr}
        else 
                ${useValgrind} ./${targetExe} > ${targetOut} 2>> ${testErr}
        fi
        rvExe=$?
        ...
        errorHeaderNmb=`wc -l < ${testErr}`
        if [ ${errorHeaderNmb} = 5 ]; then
                rm ${testErr}
        fi
